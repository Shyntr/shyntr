...
| `SAML_DEBUG_LOG_MESSAGE_BODIES` | `false` | **⚠️ Never enable in production.** When `true`, logs the raw on-the-wire SAML message body (inbound Response, inbound AuthnRequest, and issued Response) at **Debug** level with an `UNSAFE_DEBUG` warning field, to diagnose a partner exchange (e.g. ADFS). It logs the **wire form only** — the message as received or sent, **before any decryption** — so it may include the (public) XML signature and certificate, and an `EncryptedAssertion` as **ciphertext**; it never logs decrypted assertion plaintext, private keys, or client secrets. Requires `LOG_LEVEL=debug` to emit. **Scope: SAML only.** OIDC token/userinfo bodies are deliberately **excluded** from this flag because they carry access tokens and claims that are not on-the-wire-public the way a signed SAML message is. |
